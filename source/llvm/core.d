...
mixin(joinFnBinds((){
	FnBind[] ret = [
		...
		{q{LLVMMemoryBufferRef}, q{LLVMCreateMemoryBufferWithMemoryRange}, q{const(byte)* inputData, size_t inputDataLength, const(char)* bufferName, LLVMBool requiresNullTerminator}},
		{q{LLVMMemoryBufferRef}, q{LLVMCreateMemoryBufferWithMemoryRangeCopy}, q{const(byte)* inputData, size_t inputDataLength, const(char)* bufferName}},
		{q{const(byte)*}, q{LLVMGetBufferStart}, q{LLVMMemoryBufferRef memBuf}},
